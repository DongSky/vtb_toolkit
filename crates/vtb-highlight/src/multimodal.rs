...
pub fn parse_verdict(text: &str) -> Result<MultimodalVerdict> {
    ...
    let start = trimmed.find('{').ok_or(HighlightError::Config(
        "no JSON object in verdict".into(),
    ))?;
    let end = trimmed.rfind('}').ok_or(HighlightError::Config(
        "no JSON object in verdict".into(),
    ))?;
...
impl MultimodalJudge for AnthropicJudge {
    async fn judge(
        ...
        content.push(json!({
            "type": "text",
            "text": build_judge_prompt(transcript_excerpt, danmaku_excerpt),
        }));
...
pub async fn extract_frames(
    ...
    for i in 0..n {
        // Sample at (i + 0.5) / n through the span.
        let at_ms = start_ms + span * (2 * i as u64 + 1) / (2 * n as u64);
...
mod tests {
    ...
    #[test]
    fn parse_plain_json() {
        let v = parse_verdict(r#"{"score":0.85,"title":"爆笑瞬间","reason":"主播大笑"}"#)
            .unwrap();
